...
__getip() {
  ...
  if [ ! -z "$IFCONFIG" ]; then
    ...
    if [ "$IFISONLINE" -eq "0" ]; then
      CURRIP4WAN="$(curl -I4qs ifconfig.co/ip 2>/dev/null | head -1 | grep 404 >/dev/null && if [ $? == 0 ]; then curl -4qs ifconfig.co/ip 2>/dev/null; fi)"
      CURRIP6WAN="$(curl -I6qs ifconfig.co/ip 2>/dev/null | head -1 | grep 404 >/dev/null && if [ $? == 0 ]; then curl -6qs ifconfig.co/ip 2>/dev/null; fi)"
    fi
